#!/bin/bash
# Script for install Kernel and rootfs.  TIPALDI GIUSEPPE 12/2012.
# 
clear
echo "***********************************************************************"
...
rm -rf /mnt/NAND/a/*; ls -lsh /mnt/NAND/a/*; sleep 3
	...
	if [ $rd =  "y" ]; then
		...
		tar -pxvzf nand-image-debfs-armhf.tar.gz -C /mnt/NAND/b/ 
		sync
	fi
	...
	if [ $rd =  "y" ]; then
		...
		cp -v script.bin /mnt/NAND/a/script.bin
		sync
	fi
fi
