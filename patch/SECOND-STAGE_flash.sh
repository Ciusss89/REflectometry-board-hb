...
PACCHETTI="dhcp3-client udev netbase ifupdown iproute openssh-server iputils-ping wget net-tools ntpdate ntp vim nano less tzdata console-tools console-common module-init-tools cpufrequtils stress htop hdparm firmware-realtek usbutils wireless-tools bzip2 hardinfo tree lighttpd elinks bc"
...
if [ $rd = "y" ]; then
		...
		tar -jxvf /CONFIG_FILE/script.tar.bz2 -C /usr/local/; ls -lsh /usr/local/sbin/
